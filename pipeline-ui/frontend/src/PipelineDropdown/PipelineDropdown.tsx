...
const PipelineDropdown = (props: IPipelineProps) => {
  ...
  useEffect(() => {
    preparePipelines().then((p) => { setPipelines(p) });
  }, []);

  const handleChange = () => {
    const index = pipelineSelect.current?.selectedIndex || 0;
    const name = pipelineSelect.current?.options[index].value || '';
    const port = pipelineSelect.current?.options[index].getAttribute('data-port') || '';
    const protocol = pipelineSelect.current?.options[index].getAttribute('data-protocol') || '';
    setPipeline({name, protocol, port});
  }

  const optionList = pipelines.map(p => {
    const { name, protocol, port } = p;
    return <option key={name} data-protocol={protocol} data-port={port} value={name}>{name}</option>
  })

  return (
    <select id="pipeline-select" ref={pipelineSelect} className="custom-select m-1 mt-3" onChange={handleChange}>
