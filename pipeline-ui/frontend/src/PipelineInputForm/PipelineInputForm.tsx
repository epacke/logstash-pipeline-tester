...
const PipelineInputForm = (props: IPipelineInputForm) => {
  ...
  return (
    <tr>
      <td><textarea ref={logLinesTextArea} onChange={handleLogLinesChange} id="send-string"/></td>
      <td><input id="send-port" defaultValue={pipeline.port} onChange={handlePortChange}/></td>
      <td>
        <select id="send-protocol" value={pipeline.protocol} onChange={handleProtocolChange}>
...
function submitLogLines(logLines: string, port: string, protocol: string){
  fetch(
    `${BACKEND}/api/v1/sendLogLines`, {
      ...
      body: JSON.stringify({sendString: logLines, port: port, protocol: protocol})
    }
  )
}
